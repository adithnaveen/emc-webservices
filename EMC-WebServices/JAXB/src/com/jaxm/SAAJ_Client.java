package com.jaxm;

import java.io.FileOutputStream;

import javax.xml.soap.MessageFactory;
import javax.xml.soap.Name;
import javax.xml.soap.SOAPBody;
import javax.xml.soap.SOAPConnection;
import javax.xml.soap.SOAPConnectionFactory;
import javax.xml.soap.SOAPElement;
import javax.xml.soap.SOAPEnvelope;
import javax.xml.soap.SOAPMessage;

public class SAAJ_Client {
	public static void main(String[] args) throws Exception{
		FileOutputStream file = new FileOutputStream("req.xml");
		// singleton object 
		MessageFactory mFactory = MessageFactory.newInstance();
		
		SOAPMessage msg = mFactory.createMessage();
		
		SOAPEnvelope envelop = msg.getSOAPPart().getEnvelope();
		SOAPBody body = msg.getSOAPBody();
		
		Name operationElement = envelop.createName("getOneProduct", 
				"x", "http://mybasket.vinod.co/products");
		
		SOAPElement oper = body.addChildElement(operationElement);
		Name idElemnt = envelop.createName("id");
		SOAPElement id = oper.addChildElement(idElemnt);
		
		id.addTextNode("26");
		/////////////////////// End of Constructor /////////////
		SOAPConnectionFactory sFactory = SOAPConnectionFactory.newInstance();
		SOAPConnection conn = sFactory.createConnection();
		
		String address =  "http://mybasket.vinod.co/products";
		SOAPMessage res = conn.call(msg, address);
		
		res.writeTo(file);
		
		res.writeTo(System.out);
		file.close();
		
		
		
		
	}
}
